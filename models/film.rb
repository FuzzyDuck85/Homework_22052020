...
class Film
  ...
  def most_popular_screening_time()
    sql = "SELECT tickets.screening_id, COUNT (*) AS count FROM tickets INNER JOIN screenings ON tickets.screening_id = screenings.id WHERE screenings.film_id = $1 GROUP BY tickets.screening_id ORDER BY count DESC LIMIT 1"
    values = [@id]
